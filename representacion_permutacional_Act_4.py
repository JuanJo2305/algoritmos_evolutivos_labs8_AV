...
def calcular_fitness(cromosoma):
    ...
    #Contar cuántos alumnos bajos hay por grupo
    grupos_bajos = {'A': 0, 'B': 0, 'C': 0}
    for examen, indices in asignaciones.items():
        for i in indices:
            if notas[i] < 11:
                grupos_bajos[examen] += 1

    total_bajos = sum(grupos_bajos.values())

    #Penalizar si *todos* los bajos están en un solo examen
    penalizacion = 0
    if any(cuenta == total_bajos and total_bajos > 0 for cuenta in grupos_bajos.values()):
        penalizacion = 0.5
        print("⚠️ Penalización aplicada: todos los alumnos con nota < 11 en un mismo examen.")

    # Calcular desviación de promedios
    promedios = {}
    for examen, indices in asignaciones.items():
        notas_grupo = [notas[i] for i in indices]
        promedios[examen] = np.mean(notas_grupo)

    desviacion_promedios = np.std(list(promedios.values()))

    #  Bonus por diversidad interna
    bonus_diversidad = 0
    for examen, indices in asignaciones.items():
        notas_grupo = [notas[i] for i in indices]
        if max(notas_grupo) - min(notas_grupo) > 5:
            bonus_diversidad += 0.1

    # Fitness final
    fitness = -desviacion_promedios + bonus_diversidad - penalizacion
    return fitness
